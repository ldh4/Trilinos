...
TEST(nodeRegistry, test_parallel_1_0)
{
  ...
  stk::mesh::set_topology< stk::topology::HEX_20 >( block_hex_20 );
...
TEST(nodeRegistry, test_serial_hex8_tet4_24_1)
{
  ...
  stk::mesh::set_topology< stk::topology::TET_4 >( block_tet_4 );
  ...
  if (p_size <= 2)
  {
    ...
    if (p_size == 2)
    {
      EXPECT_EQ(eMesh.identifier(nodeIds_onSE_0[0]), 23u);
    }
    ...
    if (p_rank == 0)
    {
      ...
      if (p_size == 2)
      {
        EXPECT_EQ(eMesh.identifier(nodeIds_onSE_1[0]), 25u);
      }
    }
    //exit(1);
    // end_demo
