...
#define DO_IO_TESTING 1
  ...
  if (1)
  {
    Matrix rmx = rotationMatrix(0, 30);
    Matrix rmy = rotationMatrix(1, -45);
  ...
  if (1)
  {
    ...
    Matrix smx = scalingMatrix(0, scx);
    Matrix smy = scalingMatrix(1, scy);
    Matrix smz = scalingMatrix(2, scz);
    Matrix sm;
    sm =  smy * smz;
    sm =  smx * sm;
    //std::cout << "sm= " << sm << std::endl;
    transform_mesh(sm, eMesh);
