...
#include <regression_tests/HelperFunctionsCGNS.hpp>
...
void build_perturbed_cube_multiblock(PerceptMesh& eMesh, const unsigned nele,PerceptMesh& eMesh_2 , std::string filename , bool add_all_fields, bool refine = false, unsigned noRefs = 1)
{
    ...
    if(!add_all_fields) {
        ...
        bsg->register_field("coordinates_N", 3);
        bsg->register_field("coordinates_NM1", 3);
        bsg->register_field("cg_g",3);
    }
    else{
        bsg->register_field("coordinates", 3);
        bsg->register_field("coordinates_saved", 3);
        bsg->copy_field(bsg->m_fields["coordinates_saved"].get(),bsg->m_fields["coordinates"].get());
        regression_tests::add_coordinate_state_fields(bsg);
    }
    ...
    std::shared_ptr<BlockStructuredGrid> bsg_2;
    regression_tests::read_cgns_mesh(filename, bsg_2);
    ...
    if(refine)
    {
        ...
        for(unsigned iRef=0;iRef<noRefs;iRef++)
        {
            StructuredGridRefiner sgr(bsg_2, print_level);
            sgr.do_refine();
            bsg_2 = sgr.m_output;
            // TEMP
            eMesh_2.set_block_structured_grid(bsg_2);
    ...
    if(!add_all_fields) {
        bsg_2->register_field("coordinates", 3);
        bsg_2->register_field("coordinates_N", 3);
        bsg_2->register_field("coordinates_NM1", 3);
        bsg_2->register_field("cg_g",3);
    }
    else{
        bsg_2->register_field("coordinates", 3);
        bsg_2->register_field("coordinates_saved", 3);
        bsg_2->copy_field(bsg_2->m_fields["coordinates_saved"].get(),bsg_2->m_fields["coordinates"].get());
        regression_tests::add_coordinate_state_fields(bsg_2);
    }
    ...
    bsg_2->copy_field(coord_field_NM1_2, coord_field_2);
    ...
    bsg_2->copy_field(coord_field_N_2, coord_field_2);
}
...
{
    std::string fileName = "cube_4blocks.cgns";
    std::shared_ptr<BlockStructuredGrid> bsg_mb;
    regression_tests::read_cgns_mesh(fileName, bsg_mb);

    // TEMP
    PerceptMesh multiBlock(3u);
    multiBlock.set_block_structured_grid(bsg_mb);
    ...
    bsg_mb->register_field("coordinates", 3);
...
TEST(regr_PerceptCGNS, refine_smooth_cgns)
{
    stk::ParallelMachine pm = MPI_COMM_WORLD ;
    MPI_Barrier( MPI_COMM_WORLD );
    ...
    EXPECT_NEAR(multiBlock_dot, singleBlock_dot, 2.0);
}
...
void grade_mesh_along_x_axis_and_warp_y(PerceptMesh * eMesh, std::string out_name = "graded_cube_mesh", bool dump = false)
{
    ...
    bsg->copy_field(bsg->m_fields["coordinates"].get(),bsg->m_fields["coordinates_NM1"].get());
...
{   //verifies that both multi block and single block ela achieve the same results
    ...
    bsg_mb->register_field("coordinates_saved", 3);
    bsg_mb->register_field("coordinates_NM1", 3);
    bsg_mb->register_field("coordinates", 3);
    bsg_mb->register_field("cg_edge_length", 1);
    bsg_mb->register_field("num_adj_elems", 1);

    bsg_mb->copy_field(bsg_mb->m_fields["coordinates_NM1"].get(),bsg_mb->m_fields["coordinates"].get());
    bsg_mb->copy_field(bsg_mb->m_fields["coordinates_saved"].get(),bsg_mb->m_fields["coordinates"].get());
    multiBlock.nodal_field_set_value("cg_edge_length",0.0);

    bsg_sb->register_field("coordinates_saved", 3);
    bsg_sb->register_field("coordinates_NM1", 3);
    bsg_sb->register_field("cg_edge_length", 1);
    bsg_sb->register_field("num_adj_elems", 1);

    bsg_sb->copy_field(bsg_sb->m_fields["coordinates_NM1"].get(),bsg_sb->m_fields["coordinates"].get());
    bsg_sb->copy_field(bsg_sb->m_fields["coordinates_saved"].get(),bsg_sb->m_fields["coordinates"].get());
...
TEST(heavy_perceptMeshSmoother_mbvsb, bsg_field_additions)
{
    ...
    bsg_sb->register_field("alpha_field",3);
    bsg_mb->register_field("alpha_field",3);

    bsg_sb->register_field("beta_field",3);
    bsg_mb->register_field("beta_field",3);

    bsg_sb->register_field("gamma_field",3);
    bsg_mb->register_field("gamma_field",3);
...
TEST(heavy_perceptMeshSmoother_mbvsb, get_alpha_0)
{
    ...
    bsg_sb->register_field("cg_s",3);
    bsg_sb->register_field("cg_edge_length",1);

    bsg_mb->register_field("cg_s",3);
    bsg_mb->register_field("cg_edge_length",1);
...
TEST(heavy_perceptMeshSmoother_mbvsb, update_coords)
{
    ...
    regression_tests::add_coordinate_state_fields(bsg_mb);
    bsg_mb->register_field("coordinates", 3);
    ...
    std::shared_ptr<BlockStructuredGrid> bsg_sb = BlockStructuredGrid::fixture_1(singleBlock.parallel(), sizes);
    singleBlock.set_block_structured_grid(bsg_sb);
    regression_tests::add_coordinate_state_fields(bsg_sb);
