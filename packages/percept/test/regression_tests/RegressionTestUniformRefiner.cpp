...
#define DO_SMOOTHING_TEST 0
  ...
  namespace percept
  {
      extern bool s_avoidLeafConnectCheck;
...
#include "RegressionTestFileLoc.hpp"
      ...
      TEST(regr_uniformRefiner, pyramid_mesh)
      {
        EXCEPTWATCH;
        ...
        if (p_size <= 1)
          {
            ...
            {
              bool doCommit = false;
              percept::PyramidFixture mesh(MPI_COMM_WORLD, doCommit, do_sidesets);
      ...
      TEST(regr_uniformRefiner, draw1)
      {
        //std::cout << Quad4_Quad4_4::draw() << std::endl;
        std::string dir = "./";
        output_draw(dir+"quad4.dot", draw<shards::Quadrilateral<4>, shards::Quadrilateral<4>>(true) );
        output_draw(dir+"tet4.dot",  draw<shards::Tetrahedron<4>, shards::Tetrahedron<4>>() );
        output_draw(dir+"hex8.dot",  draw<shards::Hexahedron<8>, shards::Hexahedron<8>>(true) );
        output_draw(dir+"hex27.dot", draw<shards::Hexahedron<27>, shards::Hexahedron<27>>(true, true) );
        output_draw(dir+"hex20.dot", draw<shards::Hexahedron<20>, shards::Hexahedron<20>>(true, true) );
        output_draw(dir+"wedge6.dot",  draw<shards::Wedge<6>, shards::Wedge<6>>() );

        output_draw(dir+"quad9.dot", draw<shards::Quadrilateral<9>, shards::Quadrilateral<9>>(true, true));
      }

      TEST(regr_uniformRefiner, draw)
      {
        std::string dir = "./";
        output_draw(dir+"quad4.dot", draw<shards::Quadrilateral<4>, shards::Quadrilateral<4>>(true) );
        output_draw(dir+"tet4.dot",  draw<shards::Tetrahedron<4>, shards::Tetrahedron<4>>() );
      ...
      TEST(regr_uniformRefiner, tetwedge_hex)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
        ...
        if (p_size <= 1)
          {
            // create the mesh
            {
              percept::TetWedgeFixture mesh(MPI_COMM_WORLD, false, true);
        ...
        if (p_size <= 3)
          {
            ...
            percept::QuadFixture<double, stk::topology::TRI_3_2D > fixture( pm , nx , ny, true);
        ...
        if (p_size <= 3)
          {
            ...
            percept::QuadFixture<double, stk::topology::TRI_3_2D > fixture( pm , nx , ny, createEdgeSets);
      ...
      TEST(regr_uniformRefiner, wedge6_3)
      {
        EXCEPTWATCH;
      ...
      TEST(regr_uniformRefiner, heterogeneous_mesh_sidesets)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
        ...
        if (p_size <= 1)
          {
            // create the mesh
            {
              percept::HeterogeneousFixture mesh(MPI_COMM_WORLD, false, true);
      ...
      TEST(regr_uniformRefiner, heterogeneous_mesh_one_sideset_multi_topology)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
        ...
        std::string input_mesh = input_files_loc+"heterogeneous_sideset_multi_0.e";

        if (p_size <= 1)
          {
            // create the mesh
            {
              percept::HeterogeneousFixture mesh(MPI_COMM_WORLD, false, true, true);
            ...
            if (1)
              {
                ...
                eMesh1.save_as(output_files_loc+"heterogeneous_sideset_multi_1.e");
                eMesh1.close();
              }
          }
        // end_demo
      ...
      TEST(regr_uniformRefiner, pyramid_mesh_enrich)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
        ...
        if (p_size <= 1)
          {
            // create the mesh
            {
              percept::PyramidFixture mesh(MPI_COMM_WORLD, false);
              mesh.m_metaData.commit();
              mesh.populate();
        ...
        if (p_size == 1)
          {
            ...
            eMesh.setProperty("Refiner_connect_allow_duplicate_sides", "true");
            URP_Heterogeneous_3D break_pattern(eMesh);
            int scalarDimension = 0; // a scalar
            stk::mesh::FieldBase* proc_rank_field = eMesh.add_field("proc_rank", stk::topology::ELEMENT_RANK, scalarDimension);
            eMesh.commit();
      ...
      TEST(regr_uniformRefiner, tet_equi_hmesh)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
          ...
          {
            {
              ...
              unsigned npts=4;
              percept::SingleTetFixture::Point points[] = {{0,0,0},{1,0,0},{0.5,std::sqrt(3)/2.,0.0},{0.5,1./(2.0*std::sqrt(3.)), std::sqrt(2./3.)}};
              unsigned ntets=1;
              percept::SingleTetFixture::TetIds tetIds[] = {{1,2,3,4}}; // one-based

              percept::SingleTetFixture mesh(pm, false, npts, points, ntets, tetIds);
              stk::io::put_io_part_attribute(  mesh.m_block_tet );
              mesh.m_metaData.commit();
              mesh.populate();
      ...
      TEST(regr_perceptMeshSmoother, tet_4)
      {
        EXCEPTWATCH;
        stk::ParallelMachine pm = MPI_COMM_WORLD ;
        MPI_Barrier( MPI_COMM_WORLD );
                            ...
                            if (eMesh.is_valid(parent_elem))
                              {
                                if (is_matching_rank(*eMesh.m_parent_element_field, entity))
                                ...
                                else if (eMesh.m_parent_element_field_side && is_matching_rank(*eMesh.m_parent_element_field_side, entity))
                                  {
                                    ...
                                    if (fdata_new)
                                      VERIFY_OP_ON(fdata_new[0], ==, static_cast<ParentElementType_type>(predicted_parent_id), "bad parent_field "+msg);
                                  }
                              }
                          }
                      }
      ...
      TEST(regr_uniformRefiner, quad4_rebuild_family_tree)
      {
        EXCEPTWATCH;
        //if (1) return;
          ...
          {
            {
              ...
              fixture.generate_mesh();
              eMesh.save_as(output_files_loc+"quad_rft_0.e");
              eMesh.output_active_children_only(false);

              if (debug)
                {
                  std::cout << "print_all eMesh0: " << std::endl;
                  eMesh.print_all(eMesh.side_rank(), true, true);
                }

              UNIFORM_REFINER breaker(eMesh, break_quad_to_quad_4, 0);
              ...
              int nn = eMesh.get_number_nodes();
              int ne = eMesh.get_number_elements();
              if (p_rank == 0) std::cout << "num nodes = " << nn << " num_elem= " << ne << std::endl;
              eMesh.save_as(input_files_loc+"quad_rft_1.e");
              MPI_Barrier(MPI_COMM_WORLD);
              std::cout << "P[" << p_rank << " here " << std::endl;
              //eMesh.print_info("after refine", 10);
              check_parent_element_field(eMesh, "after refine", debug);

              percept::PerceptMesh eMesh1(2);
              eMesh1.set_avoid_add_all_mesh_fields_as_input_fields(true);
              eMesh1.open(input_files_loc+"quad_rft_1.e");
              eMesh1.register_and_set_refine_fields();
              eMesh1.add_registered_refine_fields_as_input_fields();
              ...
              //eMesh1.print_info("after read", 10);
              if (1)
                {
                  RefinerUtil::rebuild_family_tree(eMesh1, debug);

                  check_parent_element_field(eMesh1, "after rebuild", debug);
                  eMesh1.save_as(output_files_loc+"quad_rft_2.e");

                  if (debug)
                    {
                      std::cout << "print_all eMesh: " << std::endl;
                      eMesh.print_all(eMesh.side_rank(), true, true);
                      std::cout << "print_all eMesh1: " << std::endl;
                      eMesh1.print_all(eMesh1.side_rank(), true, true);
                    }

                  bool print_all_field_diffs = false;
                  bool print_diffs = true;
                  std::map<std::string,std::string> settings;
                  settings["ignore_auto_parts"] = "true";
                  settings["sort_buckets"] = "true";
                  ...
                  eMesh.save_as(input_files_loc+"quad_rft_diff_0.e");
                  eMesh1.save_as(input_files_loc+"quad_rft_diff_1.e");

                  if (1)
                    {
                      PerceptMesh eMesh0(2);
                      eMesh0.open_read_only(input_files_loc+"quad_rft_diff_0.e");
                      PerceptMesh eMesh01(2);
                      eMesh01.open_read_only(input_files_loc+"quad_rft_diff_1.e");

                      print_all_field_diffs = false;
                      print_diffs = true;
                      //settings["ignore_auto_parts"] = "true";
                      //settings["sort_buckets"] = "true";
                      //settings["ignore_auto_fields"] = "true";
                  ...
                  if (1)
                    {
                      ...
                      UNIFORM_REFINER breaker2(eMesh1, break_pattern);

                      breaker2.setRemoveOldElements(false);
                      breaker2.setAlwaysInitializeNodeRegistry(false);

                      breaker2.doBreak();
                      eMesh1.save_as(output_files_loc+"quad_rft_3.e");
                    }
                }
            }
            // end_demo
      ...
      TEST(regr_uniformRefiner, hex8_rebuild_family_tree)
      {
        EXCEPTWATCH;
        //if (1) return;
        bool debug = false;
        ...
        if (p_size == 1 || p_size == 3)
          {
            ...
            eMesh.output_active_children_only(false);
            //eMesh.print_info();
            eMesh.save_as(output_files_loc+"cylinder_hex8_rft_0.e");
            ...
            percept::PerceptMesh eMesh1(3u);
            eMesh1.set_avoid_add_all_mesh_fields_as_input_fields(true);
            eMesh1.open(input_files_loc+"cylinder_hex8_rft_1.e");
            eMesh1.register_and_set_refine_fields();
            eMesh1.add_registered_refine_fields_as_input_fields();

            stk::mesh::FieldBase* proc_rank_field_1 = eMesh1.add_field("proc_rank", stk::topology::ELEMENT_RANK, scalarDimension);
            stk::mesh::FieldBase* processor_id = eMesh1.add_field_int("processor_id", stk::topology::ELEMENT_RANK, scalarDimension);
            (void)proc_rank_field_1;
            (void)processor_id;

            Hex8_Hex8_8 break_pattern_1(eMesh1);
            eMesh1.commit();
            ...
            RefinerUtil::rebuild_family_tree(eMesh1, debug);
            check_parent_element_field(eMesh1, "after rebuild", debug);
            eMesh1.save_as(output_files_loc+"cylinder_hex8_rft_2.e");
            if (debug)
              {
                std::cout << "print_all eMesh: " << std::endl;
                eMesh.print_all(eMesh.side_rank(), true, true);
            ...
            eMesh.output_active_children_only(false);
            eMesh1.output_active_children_only(false);
            eMesh.save_as(input_files_loc+"hex_rft_diff_0.e");
            eMesh1.save_as(input_files_loc+"hex_rft_diff_1.e");

            if (1)
              {
                PerceptMesh eMesh0(2);
                eMesh0.open_read_only(input_files_loc+"hex_rft_diff_0.e");
                PerceptMesh eMesh01(2);
                eMesh01.open_read_only(input_files_loc+"hex_rft_diff_1.e");
            ...
            UNIFORM_REFINER breaker1(eMesh1, break_pattern_1);

            breaker.setRemoveOldElements(false);
            breaker.setAlwaysInitializeNodeRegistry(false);

            breaker.doBreak();
            eMesh1.save_as(output_files_loc+"cylinder_hex8_rft_3.e");
      ...
      TEST(regr_uniformRefiner, exposed_boundary_part)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
        ...
        std::string gmesh_spec = std::string("3x3x")+toString(p_size)+std::string("|bbox:0,0,0,1,1,"+toString(p_size)+"|sideset:xXyYzZ" );
        eMesh.new_mesh(percept::GMeshSpec(gmesh_spec));
        ...
        eMesh.commit();
        eMesh.print_info();
        eMesh.save_as("exposed_boundary_part.e");
        {
          ...
          const stk::mesh::BucketVector & buckets = eMesh.get_bulk_data()->buckets( stk::topology::FACE_RANK );
        ...
        eMesh.save_as("exposed_boundary_part.e-s0001");
