...
namespace percept
{
  namespace regression_tests
  {
    ...
    void do_smooth_block_structured(std::shared_ptr<BlockStructuredGrid> bsg, const std::string& prefix)
    {
      ...
      add_coordinate_state_fields(bsg); // for smoothing
    ...
    TEST(regr_PerceptCGNS, test6_sgrid_refine)
    {
      ...
      std::shared_ptr<BlockStructuredGrid> bsg = BlockStructuredGrid::fixture_1(MPI_COMM_WORLD, sizes);
    ...
    void do_refine_cgns_structured(const std::string& file, int print_level=0, bool allow_parallel=false)
    {
      ...
      read_cgns_mesh(file, block_structured_grid);

      StructuredGridRefiner sgr(block_structured_grid, print_level);
